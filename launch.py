...
class Config:
    ...
    def __check_args(self):
        func_count = (
            self.args.mark + self.args.review + self.args.insight + self.args.compare
        )
        if func_count > 1:
            print(
                "Only one of these four functions can be selected (mark, review, insight, compare)"
            )
            return False
        if func_count < 1:
            print(
                "One of these four functions must be selected (mark, review, insight, compare)"
            )
            return False
        if self.args.compare and self.args.cfile is None:
            print("compare function needs to specify `cfile`")
            print(
                "usage: python3 launch.py --compare --file yourfile1.csv --cfile yourfile2.csv"
            )
            return False
