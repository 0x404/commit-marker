...
class Insighter:
    ...
    def show_label_table(filename):
        ...
        label_table.field_names = ["Category", "Count", "Keyword-Related", "Keyword-NotRelated"]
        ...
        with CSVUpdater(filename, mode='r') as updater:
            for _, row in updater:
                ...
                counts[label] += 1
                if contains(subject, keywords[label]) or contains(message, keywords[label]):
                    relates[label] += 1
        label_table.add_row([CORRECTIVE, counts[CORRECTIVE], relates[CORRECTIVE], counts[CORRECTIVE] - relates[CORRECTIVE]])
        label_table.add_row([PERFECTIVE, counts[PERFECTIVE], relates[PERFECTIVE], counts[PERFECTIVE] - relates[PERFECTIVE]])
        label_table.add_row([ADAPTIVE, counts[ADAPTIVE], relates[ADAPTIVE], counts[ADAPTIVE] - relates[ADAPTIVE]])
